...
\instituicao{USP/LCB/IAG}
\titulo{Manual para os modelos: SiB2 e Balanço de água. \par Processo de calibração dos modelos pela otimização de parâmetros.}
\local{São Paulo - SP}
\data{2020}
...
Esta documentação faz referência aos repositórios: \textbf{balaguamodel}, \textbf{srcLCB} e \textbf{srcsib2model}. O repositório \textbf{balaguamodel} possui o modelo de balanço hidrológico para pequenas bacias hidrográficas proposto em \citeonline{vandewiele1992}, \textbf{srcLCB} é composto de códigos utilitários para o LCB/IAG/USP, enquanto o \textbf{srcsib2model} concentra apenas o código fonte do modelo SiB2 \cite{sellers1996I,sellers1996II}.

Os repositórios \textbf{balaguamodel}, \textbf{srcLCB} e \textbf{srcsib2model} estão versionados pelo sistema Git, portanto é possível avaliar todo o respectivo histórico de desenvolvimento de cada código usando os comandos do git.  

Logo no início destaca-se que, a versão mais atual do modelo SiB2 está no repositório \textbf{srcsib2model},
...
\chapter{Descrição e modificações no SiB2}
...
Com o código em linguagem IDL/GDL em \textbf{srcLCB}/plots/x\_plot\_checkdefault.pro é possível gerar o
...
explicitamente e todas as variáveis do tipo REAL foram declaradas com dupla precisão, i.e., 64 bits.
...
3 com o uso do f2py que integra o numpy. Para isso, uma pequena mudança
...
É importante sempre verificar os valores do ``modeloerro'' da função ``residualSiB2''. Erros enormes, principalmente associados a valores inválidos (-99999.) comprometem todo o processo de otimização de parâmetros.

Mais detalhes a respeito dos métodos de minimização de erro da classe ``Minimizer'' do LMFIT e a respeito da definição do ``modeloerro'' por meio de uma função residual, aqui denominada como ``residualSiB2'', podem ser consultadas em \url{https://lmfit.github.io/lmfit-py/}

\vspace{\baselineskip}
\subsection{Breve descrição das saídas da classe Minimizer}
\vspace{\baselineskip}
\label{descSaidaMinimizer}
...
\begin{itemize}
\item fitting method: Método de minimização do erro em função dos parâmetros.
\item function evals: Número de iterações, ie., quantas vezes a função residual foi chamada.
\item data points: Tamanho da série de dados.
 \item variables: São os parâmetros do modelo a ser calibrado.     
\item chi-square: Somatório do erro quadrático do conjunto de parâmetros ótimos.
\item reduced chi-square: $\chi^2$ / {(Número de pontos da série de dados) - (Número de parâmetros)}
\item Akaike info crit: Akaike Information Criterion statistic (aic)
\item Bayesian info crit: Bayesian Information Criterion statistic (bic)
\item {[[Variables]]}: O resultados dos parâmetros ótimos segue com o erro (+/-) que é calculado com base na matriz de covariância dos parâmetros e ``init ='' é o valor inicial do parâmetro.
\item {[[Correlations]]}: Correlação linear de cada par de parâmetros a partir da matriz de covariância dos parâmetros.
\end{itemize}

Os melhores resultados de conjunto de parâmetros estarão associados com menores valores de chi-square ($\chi^2$), aic e bic. Para mais informações a respeito das estatísticas presentes nos saídas da ``Minimizer'', consulte \url{https://lmfit.github.io/lmfit-py/fitting.html}.
...
A partir do \textit{Distributed Biosphere-Hydrological (DBH) Model} disponível em \url{http://hydro.iis.u-tokyo.ac.jp/DBH/}, o qual tem interface com o SiB2, utilizou-se a função ``derive\_trans.f'' para o cálculo dos parâmetros aerodinâmicos relacionados as características da cobertura vegetal dos biomas.

A ``derive\_trans.f'' do DBHM foi traduzida para Fortran 95 (``derive\_trans.f95''), e acoplada no SiB2 para que o conjunto de parâmetros aerodinâmicos seja calculado conforme a variação do índice de área foliar a cada passo de tempo, sobrescrevendo os valores fixados do data1.

O SiB2 acoplado com a ``derive\_trans'' está em \textbf{srcsib2model}/SiB2\_DBHM\_derive\_trans/F95. O DBHM também encontra-se no repositório em \textbf{srcsib2model}/SiB2\_DBHM\_derive\_trans/DBHModel.

A função ``derive\_trans'' acoplada no SiB2 recebe os parâmetros
\begin{Verbatim}[breaklines=true]
...
Obs.: O programa callSiB2pymodule.py em \textbf{srcsib2model}/SiB2pymod/RadiativeTransModule, é um exemplo de como o sib2pymod do módulo de radiação deve ser executado no ambiente Python. 
...
\item Informe o número de linhas de dados do arquivo data2 no programa ``callSiB2pymodule.py'' e execute-o.
\begin{Verbatim}[breaklines=true]
...
\section{Considerações sobre a calibração do SiB2 usando o método bruto}

Apesar do método ``brute'' de minimização do qui-quadrado -- que é somatório do erro quadrático $\chi^2 = \sum_{i}^{N} (Resid_i)^2$ (descrito também em \ref{descSaidaMinimizer}) -- possuir um custo computacional alto e portanto exige um bom dimensionamento da grade de parâmetros a ser investigada, nele é calculado o $\chi^2$ para todo o espaço definido pela grade de parâmetros, possibilitando uma visualização gráfica de $\chi^2$ em função dos parâmetros. Este tipo visualização (ver figure \ref{gridsearch}) é uma cópia do exemplo \url{https://lmfit.github.io/lmfit-py/examples/example_brute.html?highlight=brute%20example} contido na documentação do LMFIT e é gerado a partir do programa ``loadBruteGridMinimizerResult.py'' onde a função ``gradeparamsbrute.py'' é chamada. O programa ``loadBruteGridMinimizerResult.py'' apenas restaura o resultado gerado em ``calibraSiB2\_brute.py'' e faz os gráficos por meio da função ``gradeparamsbrute.py''. O programa ``loadBruteGridMinimizerResult.py'' e a função ``gradeparamsbrute.py'' estão contidos  no diretório de cada módulo de calibração.
...
O programa ``loadMinimizerResult\_checkCandidates.py'' ou ``loadPlotGridParams.py'' em \textbf{balaguamodel} é uma adaptação do exemplo denominado como \textit{Global minimization using the brute method (a.k.a. grid search)}, o qual faz uma avaliação das saídas da classe ``Minimizer'' para o método ``brute''. Este tipo de avaliação permite uma visualização gráfica das soluções conforme mostra a figura \ref{gridsearch}. Para mais detalhes, consulte 
\url{https://lmfit.github.io/lmfit-py/examples/example_brute.html?highlight=brute%20example}

\begin{figure}[ht]
\centering 
\includegraphics[width=\textwidth]{img/4B-015_pltGridParams}
\caption{Visualização gráfica dos valores dos parâmetros associados ao erro mínimo, com base nos cálculos do método ``brute'' da classe ``Minimizer'' do python LMFIT.}
\label{gridsearch}
\end{figure}
